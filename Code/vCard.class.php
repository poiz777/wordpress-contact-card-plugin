...
class vCard {
    ...
    function build()
    {
        ...
        if ($this->data['company']) {
            $this->card .= "ORG:" . $this->data['role'] . ";" . $this->data['company'];
        }
        if ($this->data['department']) {
  	    ...
  	    if ($this->data['home_po_box'] || $this->data['home_extended_address']
   	    ...
   	    if ($this->data['photo']) {
            $type       = strtoupper( pathinfo($this->data['photo'], PATHINFO_EXTENSION) );
            $mediaType  = "image/" . pathinfo($this->data['photo'], PATHINFO_EXTENSION);

            $this->card .= "PHOTO;" . $type . ":" . $this->data['photo'] . "\r\n";
            $this->card .= "PHOTO;VALUE=URI;TYPE={$type}:" . $this->data['photo'] . "\r\n";     // V: 3
            $this->card .= "PHOTO;MEDIATYPE=image/gif={$mediaType}:" . $this->data['photo'] . "\r\n";     // V: 3
            $this->card .= "PHOTO;ENCODING=b:" . $this->encodePhoto($this->data['photo']) . "\r\n";
   	    }
   	    $this->card .= "TZ:" . $this->data['timezone'] . "\r\n";
   	    $this->card .= "END:VCARD\r\n";
    }

    function encodePhoto($pixURL){
        $type       = pathinfo($pixURL, PATHINFO_EXTENSION);
        $data       = file_get_contents($pixURL);
    ...
    function download()
    {
        ...
        $this->filename = str_replace(' ', '_', $this->filename);   //Content-Type: text/x-vcard; charset=iso-8859-1

        header("Content-type: text/directory; charset=iso-8859-1");
